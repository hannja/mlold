\l /home/james/noodlings_priv/ml/pyutils.q
.py.imp_class_from[`keras.callbacks]each `Callback`ModelCheckpoint`TensorBoard
.py.imp_class_from[`keras.layers]each `Dense`Dropout`Embedding`LSTM`TimeDistributed
...
dd:(0#`)!()
/ everything seems to have more than 8 params, so just treat params as dict
/ validation and apply defaults
/ signals error or returns defaults overrriden with param dict given
vldparams:{[req;ps;defs]
 if[not all u:reqp in key ps:defs,ps;'"missing params: ",csv sv string reqp where not u];
 ps}
build_model:{[p]
 p:vldparams[key[defs],`batchsz`seqlen;p]defs:select vocsz:TODO,embedsz:32,rnnsz:128,nl:2,dr:0.,lr:.001,cn:5. from dd; 

 model:.py.classinstance Sequential[]; / we'll be using methods from model
 model.add each Embedding[p`vocsz;p`embedsz;`batch_input_shape pp p`batchsz`seqlen],Dropout p`dr;
 / add LSTM layers
 model.add each raze {[rnnsz;dr](LSTM[rnnsz;`return_sequences pp 1b;`stateful pp 1b];Dropout dr)}[p`rnnsz]each p[`nl]#p`dr;
 model.add TimeDistributed Dense[p`vocsz;`activation pp `softmax];
 model.compile[`loss pp `categorical_cross_entropy;`optimizer pp Adam[p`lr;`clipnorm pp p`cn];
 model}

buildinferencemodel:{[p]
 p:vldparams[key[defs],`model;p]defs:select batchsz:1,seqlen:1 from dd;
 / grr no dot notation with locals;
 model:p`model;
 config:.P.GET model[`get_config][];
 config[0;`config;`batch_input_shape]:p`batchsz`seqlen;
 inference_model:model.from_config config;
 / TODO maybe should be dict wrapped if going to be used
 / if we do end up wrapping it can do inference_model[`trainable][:;0b];
 .py.setattr[inference_model;`trainable;0b];
 inference_model}
generate_text:{[model;seed;p]
 p:vldparams[key defs;p]defs:select len:512,topn:10 from dd;
 len:p`len;topn:p`topn;
 generated:seed;
 encoded:enctext seed; / encode_text? what does it do, should do in q
 model[`reset_states][];
